...
#define SNP_TRC_TIMESTAMP_VALUE 100

#if (SNP_TRC_TIMESTAMP_VALUE<1) || (SNP_TRC_TIMESTAMP_VALUE>1000)
#error "Bad value for timestamp base"
#endif

// Trace buffer size in TraceBufferFieldT words; recommended to be a multiple of the trace
...
_NEAR_DATA_ static TraceBufferFieldT g_dataTraceBuffer[SNP_TRC_BUFFER_SIZE];
_NEAR_DATA_ static volatile DataTraceHeaderT g_traceHeader=
{
  // Static fields (affected at compilation time)
  0xA7, 0x25, 0x8F,                          // startMark[3]: Unchanging marker used for robustness
  sizeof(TraceHeaderFieldT),                 // Size in bytes of following fields in the header
  ...
  0, // Correct value will be initialized at runtime
#else
  (TraceHeaderFieldT) &g_dataTraceBuffer,        // bufferStartAddr:  First (low) address of memory area reserved for trace data
  (TraceHeaderFieldT) (&g_dataTraceBuffer[SNP_TRC_BUFFER_SIZE])-1, // bufferEndAddr:    Last (high) address of memory area reserved for trace data
#endif
  ...
  0, // Correct value will be initialized at runtime
#else
  (TraceHeaderFieldT) &g_dataTraceBuffer,                        // writePointer:     Position in the trace buffer where to put the next record
  (TraceHeaderFieldT) &g_dataTraceBuffer,                        // writePointerCopy: Do a copy as basic security for shared area
  (TraceHeaderFieldT) (&g_dataTraceBuffer[SNP_TRC_BUFFER_SIZE]-1), // readPointer:      Position in the trace buffer of the last fully read record
  (TraceHeaderFieldT) (&g_dataTraceBuffer[SNP_TRC_BUFFER_SIZE]-1), // readPointerCopy:  Do a copy as basic security for shared area
        // CAUTION: this initial value is not aligned with a start of record address; the effective value has to be computed later in order
        // to correctly detect the first buffer overflow
#endif
...
static unsigned long g_recordSize=0; // In bytes
static TraceHeaderFieldT g_lastReadPointer = 0;

#ifdef STM32F0XX
// Align the variable's addresses
void alignAddresses(unsigned long nbVar);
#endif
...
{
  g_traceHeader.bufferStartAddr = (TraceHeaderFieldT) &g_dataTraceBuffer;
  g_traceHeader.bufferEndAddr = (TraceHeaderFieldT) (&g_dataTraceBuffer[SNP_TRC_BUFFER_SIZE])-1;
  g_traceHeader.writePointer = (TraceHeaderFieldT) &g_dataTraceBuffer;
  g_traceHeader.writePointerCopy = (TraceHeaderFieldT) &g_dataTraceBuffer;
  g_traceHeader.readPointer = (TraceHeaderFieldT) (&g_dataTraceBuffer[SNP_TRC_BUFFER_SIZE]-1);
  g_traceHeader.readPointerCopy = (TraceHeaderFieldT) (&g_dataTraceBuffer[SNP_TRC_BUFFER_SIZE]-1);
}
#endif

typedef TraceBufferFieldT (*pReadFunc)(unsigned long);
...
// The data trace dump function that must be called by the application where a
...
void DumpTrace(void)
{
	static char bOverflow=0;
	static char bStarted=0;
	static TraceBufferFieldT *pWrite=0;
...
//	int timeout;
	static unsigned long nbRecords=0; // static because reused (instead of recomputed) by triggers
	TraceHeaderFieldT expectedInitialReadPointer;
	TraceHeaderFieldT flags, writePointer;
	unsigned short nbVar, nbDoubleVar;
#ifdef USING_TRIGGER
	static char bHasTrigged = 0;
	int trigEval;
#endif
